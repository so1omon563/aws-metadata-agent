...
The tap is third-party executable code. Review it before trusting it, then run:
...
`brew install` never invokes `sudo`, changes network state, or loads services.
`aws-metadata setup` runs the same reviewed installer used by source releases.
If `aws-runas` is absent, setup invokes the checksum-verified bootstrap, which
downloads the unmodified binary directly from the official upstream release.
The formula does not bundle or mirror `aws-runas`.
